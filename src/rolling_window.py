...
def run_rolling_window_pipeline(conn, user_id="system_default", batch_id=None):
    ...
    if not df_history.empty:
        ...
        for r in hist_records:
            ...
            # Extract values safely converting nan/None to float/int/None
            ...
            # Update lifetime stats first (independent of whether date is missing)
            ...
            # If date is missing/NaT, skip adding it to sliding window history
            if pd.isna(dt):
                continue
    ...
    for row in active_records:
        ...
        # Lifetime (O(1) Running Totals) are extracted here so they are always defined
        ...
        # Update histories only if date is not missing
        ...
        if pd.isna(current_date):
            # If current transaction has no date, rolling features are all 0/default
            cust_txn_count = 0
            cust_flags = 0
            cust_avg_delay = 0.0
            ...
            p_dates = proj_dates.get(proj_id, [])
            p_entries = proj_hist.get(proj_id, [])
